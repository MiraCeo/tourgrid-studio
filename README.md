...
- 浏览器本地图片读取、正方形裁切、亮度/对比度/饱和度与颜色滤镜实时预览；
  提供像素画局部代表色、照片线性光面积平均，以及面积平均约束的边缘自适应
  Mitchell 三种采样方式；主预览在处理后原图与
...
- 移动端横屏保留紧凑三栏与专注抽屉；竖屏自动切换为画布优先布局，通过底部
  “参考 / 工具 / 色板”入口互斥打开面板。浏览器支持时可尝试全屏锁定横屏，
  不支持时仍可继续完成竖屏编辑。
...
- 主界面“发现”弹窗集中展示管理员配置的推荐作品池、项目公告与使用帮助；
  推荐池在浏览器本地洗牌，每批展示最多6个作品，网格上下均可换一批；推荐预览
  不会增加浏览次数，读取作品时沿用现有画布替换确认。
- 推荐卡片支持匿名点赞；点赞与读取分别按同一30分钟窗口去重，并共同累加现有浏览量，
  不保存独立点赞记录。
- 管理后台可按分享码维护不限数量的全站推荐作品池。
