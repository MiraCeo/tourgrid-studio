...
2. FastAPI负责色板查询、不可变作品保存、凭码读取和推荐作品配置。
...
- 照片自然采样与像素画保色采样、双向主预览、独立中间采样参考和
  像素画对齐网格；
- “细节（平衡）”模式已经接入，独立浏览器回归基准留待后续补充；
- 目标颜色缩减后的固定色板约束；
...
- 导入、清空和参考图的完整撤销恢复；
- 24×24 PNG及16倍最近邻图导出；
- 作品发布、分享码复制、凭码读取，以及“发现”弹窗的推荐作品入口；
...
- 管理员可保存最多6个有序推荐作品；公开推荐预览不增加浏览次数，隐藏作品自动跳过；
- 未配置数据库时返回统一503错误；
