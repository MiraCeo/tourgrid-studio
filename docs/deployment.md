...
`ports` 映射。
PostgreSQL 只绑定宿主机回环地址，便于本机维护和集成测试，不对公网开放。
作品数据保存在 `postgres_data` 卷中。
...
作品保存与凭码读取、重复作品去重、移动端裁切、请求限流和错误日志。
...
4. 验证 `/api/v1/health`、一次固定图片转换、作品凭码读取和前端导出。
5. 保留失败版本的容器日志用于排查。
...
- `POST /api/v1/convert` 和 `POST /api/v1/works` 使用按客户端 IP 的进程内滑动窗口限流。
- 限流器会定期清除窗口外的客户端，并通过
...
- `TOURGRID_DB_PASSWORD` 必须使用独立强密码；数据库只允许内网和宿主机回环访问。
- 定期使用 `pg_dump` 备份作品表，并在独立位置验证恢复流程；“永久保存”依赖可恢复备份。
